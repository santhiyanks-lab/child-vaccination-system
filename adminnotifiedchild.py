...
# ---- Notified children (for main table) ----
cur.execute("""
SELECT DISTINCT c.child_id, c.child_name, c.dob, c.weight,
...
WHERE c.child_id IN (SELECT DISTINCT child_id FROM child_vaccine)
ORDER BY c.child_id
""")
children = cur.fetchall()

# ---- All children (for children details section) ----
cur.execute("""
...
<head>
  ...
  <style>
    *,*::before,*::after{{margin:0;padding:0;box-sizing:border-box}}
    ...
    .avatar-circle{{width:32px;height:32px;border-radius:50%;background:var(--wl);color:var(--wa);display:flex;align-items:center;justify-content:center;font-size:.78rem;font-weight:700;flex-shrink:0}}
    ...
    .badge-notified{{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;background:var(--wl);color:var(--wa);border-radius:20px;font-size:.78rem;font-weight:700}}
    .badge-pending{{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;background:var(--pl);color:var(--p);border-radius:20px;font-size:.78rem;font-weight:700}}
    .badge-confirmed{{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;background:#e8f5e9;color:#2e7d32;border-radius:20px;font-size:.78rem;font-weight:700}}
    .badge-rescheduled{{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;background:#fff3e0;color:#e65100;border-radius:20px;font-size:.78rem;font-weight:700}}
    .badge-completed{{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;background:#e8f5e9;color:#1b5e20;border-radius:20px;font-size:.78rem;font-weight:700}}
    ...
    .modal-box{{background:var(--ca);border-radius:var(--ra);width:100%;max-width:700px;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 8px 40px rgba(0,0,0,.2);animation:slideUp .25s ease}}
    @keyframes slideUp{{from{{transform:translateY(40px);opacity:0}}to{{transform:translateY(0);opacity:1}}}}
    .modal-head{{background:linear-gradient(90deg,#e65100,var(--wa));color:#fff;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;border-radius:var(--ra) var(--ra) 0 0;flex-shrink:0}}
    ...
    .vac-badge-confirmed{{background:#e8f5e9;color:#2e7d32;padding:2px 8px;border-radius:10px;font-size:.74rem;font-weight:700}}
    .vac-badge-rescheduled{{background:#fff3e0;color:#e65100;padding:2px 8px;border-radius:10px;font-size:.74rem;font-weight:700}}
    .vac-badge-completed{{background:#e8f5e9;color:#1b5e20;padding:2px 8px;border-radius:10px;font-size:.74rem;font-weight:700}}
    /* ===== CHILDREN DETAILS ===== */
    ...
    @media(max-width:768px){{
      ...
      .sbox input{{width:160px}}.detail-tabs{{overflow-x:auto}}
    }}
  ...
  <nav class="snav">
    ...
    <div class="ng open" id="g4">
      ...
      <div class="nsub">
        ...
        <a href="adminnotifiedchild.py" class="active"><i class="fa-solid fa-child"></i> Notified Child</a>
...
<div class="mwrap">
  <header class="topbar">
    <div class="tbl">
      ...
      <div>
        <div class="ttitle" id="page-title">Notified Children</div>
        <div class="tbcrumb">
          <a href="admindashboard.py">Dashboard</a>
          <span>&rsaquo;</span>
          <span id="breadcrumb-sub">Notification &rsaquo; Notified Child</span>
        </div>
      </div>
    </div>
    <div class="tbr">
  ...
  <main class="pc">
    ...
    <!-- ===== SECTION: NOTIFIED TABLE ===== -->
    <div class="section active" id="sec-notified">
      <div class="page-hdr">
        <div class="page-hdr-left">
          <div class="page-hdr-icon"><i class="fa-solid fa-bell"></i></div>
          <div>
            <h2>Notified Children</h2>
            <p>Children who have received vaccination notifications.</p>
          </div>
        </div>
        <span class="cnt-badge" id="rowCount"><i class="fa-solid fa-bell"></i> -- Notified</span>
      </div>
      <div class="info-banner">
        <i class="fa-solid fa-circle-info"></i>
        <span>These children have been notified. Their vaccine status updates automatically as the hospital confirms or reschedules. Go to <a href="adminnotification.py" style="color:var(--wa);font-weight:700;">Notify</a> to send new notifications.</span>
      </div>
      <div class="toolbar">
        <div class="sbox">
          <i class="fa-solid fa-magnifying-glass"></i>
          <input type="text" id="searchInput" placeholder="Search children..." oninput="filterTable()">
        </div>
      </div>
      <div class="tcard">
        <div class="tcard-head"><i class="fa-solid fa-bell"></i> Notified Children List</div>
        <div class="tw">
          <table id="mainTable">
            <thead>
              <tr>
...
if children:
    for idx, child in enumerate(children, start=1):
        ...
        cur.execute("SELECT status FROM child_vaccine WHERE child_id=%s ORDER BY id DESC LIMIT 1", (child_id,))
        latest = cur.fetchone()
        latest_status = str(latest[0]).strip().lower() if latest else "notified"

        if latest_status == "notified":
            badge = f'<span class="badge-notified"><i class="fa-solid fa-bell"></i> Notified</span>'
        elif latest_status == "pending":
            badge = f'<span class="badge-pending"><i class="fa-solid fa-clock"></i> Pending</span>'
        elif latest_status == "confirmed":
            badge = f'<span class="badge-confirmed"><i class="fa-solid fa-check"></i> Confirmed</span>'
        elif latest_status == "rescheduled":
            badge = f'<span class="badge-rescheduled"><i class="fa-solid fa-calendar-days"></i> Rescheduled</span>'
        elif latest_status == "completed":
            badge = f'<span class="badge-completed"><i class="fa-solid fa-circle-check"></i> Completed</span>'
        else:
            badge = f'<span class="badge-notified"><i class="fa-solid fa-bell"></i> {latest_status.capitalize()}</span>'

        print(f"""
              <tr>
                <td><span class="snum">{idx}</span></td>
                <td><div class="child-avatar"><div class="avatar-circle">{initials}</div><strong>{child_name}</strong></div></td>
                <td>{child_dob}</td>
                <td>{weight} kg</td>
                <td>{badge}</td>
                <td><button class="btn-view" onclick="openModal('modal{child_id}')"><i class="fa-solid fa-eye"></i> View More</button></td>
              </tr>""")
else:
    print("""<tr><td colspan="6"><div class="empty-state">
      <i class="fa-solid fa-bell-slash"></i>
      <h3>No Notified Children</h3><p>No vaccination notifications have been sent yet.</p>
    </div></td></tr>""")
...
print(f"""
    <div class="section" id="sec-children">
      ...
      <div class="ctoolbar">
        ...
        <span class="cnt-lbl" id="cntLbl">{len(all_children)} records</span>
      </div>
""")
...
for child in children:
    ...
    cur.execute("""
        SELECT vaccine_id, vaccine_name, minimum_age, dose_number FROM vaccine
        WHERE status='confirmed'
        ORDER BY COALESCE(minimum_age,0) ASC, COALESCE(dose_number,1) ASC LIMIT 10
    """)
    ...
    for vac in vaccines:
        ...
        try:
            scheduled_date = add_months(child_dob, int(minimum_age) if minimum_age else 0)
        except Exception:
            scheduled_date = child_dob

        cur.execute("""
            SELECT cv.appointment_date, cv.taken_date, cv.status, cv.dose_number, h.hospital_name
            FROM child_vaccine cv
            LEFT JOIN hospital h ON cv.hospital_id = h.hospital_id
            WHERE cv.child_id=%s AND cv.vaccine_id=%s LIMIT 1
        """, (child_id, vaccine_id))
        record = cur.fetchone()

        if record:
            v_status     = str(record[2]).strip().lower() if record[2] else "notified"
            rec_dose     = record[3] if record[3] else dose_number
            hosp_display = record[4] if record[4] else "Not Assigned"
            display_date = record[1] if record[1] else (record[0] if record[0] else scheduled_date)
            if v_status == 'completed':   badge_cls,label = "vac-badge-completed","Completed"
            elif v_status == 'confirmed': badge_cls,label = "vac-badge-confirmed","Confirmed"
            elif v_status == 'notified':  badge_cls,label = "vac-badge-notified","Notified"
            elif v_status == 'rescheduled': badge_cls,label = "vac-badge-rescheduled","Rescheduled"
            else:                         badge_cls,label = "vac-badge-pending",v_status.capitalize()
        else:
            display_date,rec_dose,hosp_display = scheduled_date,dose_number,"Not yet assigned"
            badge_cls,label = "vac-badge-pending","Not Notified"

        vac_rows += (f"<tr><td>{vaccine_name}</td><td><b>Dose {rec_dose}</b></td>"
                     f"<td>{display_date}</td><td>{hosp_display}</td>"
                     f"<td><span class='{badge_cls}'>{label}</span></td></tr>")

    print(f"""
<div class="modal-overlay" id="modal{child_id}">
  <div class="modal-box">
    <div class="modal-head">
      <h4><i class="fa-solid fa-child"></i> {child_name} — Full Details</h4>
    ...
    <div class="modal-body">
      ...
      <div class="img-row">
        <div class="img-item"><img src="./images/{child[19]}" class="modal-img" alt="Aadhar" onerror="this.src='images/noimage.png'"><p>Father Aadhar</p></div>
        <div class="img-item"><img src="./images/{child[20]}" class="modal-img" alt="Profile" onerror="this.src='images/noimage.png'"><p>Parent Profile</p></div>
      </div>
      <div class="modal-section-title"><i class="fa-solid fa-syringe"></i> Vaccine Schedule</div>
      <div style="overflow-x:auto;">
        <table class="vac-table">
          <thead><tr><th>Vaccine</th><th>Dose</th><th>Appointment Date</th><th>Hospital</th><th>Status</th></tr></thead>
...
function showSection(id) {
  ...
  const titles     = { 'sec-notified': 'Notified Children', 'sec-children': 'Child Details' };
  const breadcrumbs = { 'sec-notified': 'Notification &rsaquo; Notified Child', 'sec-children': 'Children &rsaquo; Child Details' };
  document.getElementById('page-title').textContent = titles[id] || 'Notified Children';
...
document.getElementById('rowCount').innerHTML = '<i class="fa-solid fa-bell"></i> ' + allRows.length + ' Notified';
...
function filterTable() {
  ...
  document.getElementById('rowCount').innerHTML = '<i class="fa-solid fa-bell"></i> ' + visible + ' Notified';
}
