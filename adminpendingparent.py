...
import sys, cgi, cgitb, pymysql, smtplib
...
if approve and cid:
    cur.execute("SELECT father_name, email, username, password FROM parent WHERE parent_id=%s", (cid,))
    data = cur.fetchone()
    if data:
        name, email, saved_user, saved_pass = data
        cur.execute("UPDATE parent SET status='approved' WHERE parent_id=%s", (cid,))
        con.commit()
        body = (f"Hello {name},\n\nYour Parent Account has been Approved.\n\n"
                f"Username : {saved_user}\nPassword : {saved_pass}\n\n"
                f"Thank you,\nChild Vaccination System")
        ok, err = send_email(email, "Parent Account Approved - Child Vaccination System", body)
        msg = "Parent Approved and Email Sent!" if ok else f"Parent Approved but Email Failed: {err}"
        safe_msg = msg.replace("'", "\\'").replace('"', '\\"')
        redirect_script = f'<script>alert("{safe_msg}");window.location.href="adminpendingparent.py";</script>'

elif reject and cid:
    cur.execute("SELECT father_name, email FROM parent WHERE parent_id=%s", (cid,))
    data = cur.fetchone()
    if data:
        name, email = data
        cur.execute("UPDATE parent SET status='rejected' WHERE parent_id=%s", (cid,))
        con.commit()
        body = (f"Hello {name},\n\nYour registration has been rejected.\n"
                f"You may reapply with correct documents.\n\nThank you,\nChild Vaccination System")
        ok, err = send_email(email, "Parent Registration Update - Child Vaccination System", body)
        msg = "Parent Rejected and Email Sent!" if ok else f"Parent Rejected but Email Failed: {err}"
        safe_msg = msg.replace("'", "\\'").replace('"', '\\"')
        redirect_script = f'<script>alert("{safe_msg}");window.location.href="adminpendingparent.py";</script>'

cur.execute("SELECT * FROM parent WHERE status='pending'")
rows = cur.fetchall()

# State and district options for filter bar
cur.execute("SELECT DISTINCT state FROM parent WHERE status='pending' AND state IS NOT NULL ORDER BY state")
pending_states = [r[0] for r in cur.fetchall()]

cur.execute("SELECT DISTINCT state, district FROM parent WHERE status='pending' AND state IS NOT NULL AND district IS NOT NULL ORDER BY state, district")
...
child_ids_all = [r[0] for r in all_children]
...
<head>
  ...
  <style>
    *,*::before,*::after{{margin:0;padding:0;box-sizing:border-box}}
    ...
    .cnt-lbl-bar{{font-size:.83rem;color:var(--mu);font-weight:600;white-space:nowrap;margin-left:auto}}
    /* ── Table ── */
    ...
    .parent-cell{{display:inline-flex;align-items:center;gap:9px}}
    ...
    .btn-view{{display:inline-flex;align-items:center;gap:6px;padding:7px 14px;background:var(--pl);color:var(--p);border:1px solid var(--p);border-radius:7px;font-family:var(--fn);font-size:.82rem;font-weight:600;cursor:pointer;transition:background var(--tr),color var(--tr)}}
    ...
    .modal-box{{background:var(--ca);border-radius:var(--ra);width:100%;max-width:760px;max-height:92vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,.2);animation:slideUp .25s ease}}
    ...
    .ch-table{{width:100%;border-collapse:collapse;font-size:.82rem;margin-top:4px}}
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="snav">
    <div class="ng open" id="g1">
      ...
      <div class="nsub">
        <a href="adminpendingparent.py" class="active"><i class="fa-solid fa-clock"></i> Pending {sb(pending_parents)}</a>
...
<div class="mwrap">
  <header class="topbar">
    <div class="tbl">
      ...
      <div>
        <div class="ttitle" id="page-title">Pending Parents</div>
        <div class="tbcrumb">
          <a href="admin_dash.py">Dashboard</a><span>&rsaquo;</span>
          <span id="breadcrumb-sub">Parents &rsaquo; Pending</span>
        </div>
      </div>
    </div>
    <div class="tbr">
    ...
    <div class="section active" id="sec-pending">
      <div class="page-hdr">
        <div class="page-hdr-left">
          <div class="page-hdr-icon"><i class="fa-solid fa-user-clock"></i></div>
          <div>
            <h2>Pending Parents</h2>
            <p>Filter by state and district, or search by name / email / mobile.</p>
          </div>
        </div>
      ...
      <div class="filter-bar">
        ...
        <span class="cnt-lbl-bar" id="filterCount">{len(rows)} records</span>
      </div>

      <div class="tcard">
        <div class="tcard-head"><i class="fa-solid fa-clock"></i> Pending Parent List</div>
        <div class="tw">
          <table id="mainTable">
            <thead>
              <tr>
...
if rows:
    for idx, row in enumerate(rows, start=1):
        ...
        state       = row[9]  or ""
        district    = row[10] or ""
        initials    = father_name[0].upper() if father_name else "P"
        print(f"""
              <tr data-state="{state.lower()}" data-district="{district.lower()}">
                <td><span class="snum">{idx}</span></td>
                <td><div class="parent-cell"><div class="avatar-circle">{initials}</div><strong>{father_name}</strong></div></td>
                <td>{mother_name}</td>
                ...
                <td><button class="btn-view" onclick="openModal('modal{parent_id}')"><i class="fa-solid fa-eye"></i> View &amp; Decide</button></td>
              </tr>""")
else:
    print("""
              <tr><td colspan="9">
                <div class="empty-state">
                  ...
                  <h3>No Pending Parents</h3>
                  <p>All parent registrations have been reviewed.</p>
                </div>
              </td></tr>""")
    ...
    <div class="section" id="sec-children">
      <div class="page-hdr2">
        <div>
          ...
          <p>Expand any child to see parent, vaccine &amp; full cross-hospital dose history</p>
        </div>
...
for r in all_children:
    ...
    if cross_doses:
        ...
        for d in cross_doses:
            hname = d["hospital_name"]
            if hname not in hosp_groups:
                hosp_groups[hname] = {"address": d["address"], "doses": []}
            hosp_groups[hname]["doses"].append(d)
        ...
        ch_alert = (
            ...
            f'<strong>{len(hosp_groups)} other hospital(s)</strong> before the current appointment.</span>'
            f'</div>'
        )
        ch_rows = ""
        for d in cross_doses:
        ...
        cross_badge = f'&nbsp;<span class="badge-cross"><i class="fa-solid fa-hospital" style="font-size:.68rem;"></i> {cross_count} other hosp.</span>'
    else:
        cross_tab_html = '<div class="no-ch"><i class="fa-solid fa-circle-check" style="color:#1d9e75;margin-right:6px;"></i>No doses from other hospitals recorded.</div>'
        cross_badge = ""
      ...
      <div class="child-card"
        ...
        <div class="ccard-details" id="cdet{cid}">
          <div class="detail-tabs" id="ctabs{cid}">
            ...
            <button class="dtab" onclick="switchDTab({cid},'crosshosp')" style="{'color:#e65100;' if cross_count > 0 else ''}">
              <i class="fa-solid fa-code-branch"></i> Other Hosp. Doses
              {'<span class="badge-cross" style="margin-left:4px;">' + str(cross_count) + '</span>' if cross_count > 0 else ''}
            </button>
          </div>
          ...
          <div class="dpanel" id="cdp{cid}-crosshosp">
            {cross_tab_html}
          </div>
        </div>
      </div>
...
for row in rows:
    ...
    address             = row[11]
    pincode             = row[12]
    father_aadhar_image = row[13]
    parent_profile      = row[14]
    username            = row[15] or ""
    password            = row[16] or ""

    print(f"""
<div class="modal-overlay" id="modal{parent_id}">
  <div class="modal-box">
    <div class="modal-head">
      <h4><i class="fa-solid fa-user"></i> {father_name} &mdash; Parent Details</h4>
      <button class="modal-close" onclick="closeModal('modal{parent_id}')">&#x2715;</button>
    </div>
    <form method="post" action="adminpendingparent.py">
      <div class="modal-body">
        <div class="modal-section-title"><i class="fa-solid fa-user"></i> Father Information</div>
        <div class="detail-grid">
          <div class="detail-item"><label>Father Name</label><span>{father_name}</span></div>
          <div class="detail-item"><label>Date of Birth</label><span>{father_age}</span></div>
          <div class="detail-item"><label>Occupation</label><span>{occupation}</span></div>
          <div class="detail-item"><label>Email</label><span>{email}</span></div>
          <div class="detail-item"><label>Mobile</label><span>{mobile_number}</span></div>
        </div>
        <div class="modal-section-title"><i class="fa-solid fa-user"></i> Mother Information</div>
        <div class="detail-grid">
          <div class="detail-item"><label>Mother Name</label><span>{mother_name}</span></div>
          <div class="detail-item"><label>Date of Birth</label><span>{mother_age}</span></div>
          <div class="detail-item"><label>Mother Weight</label><span>{mother_weight} kg</span></div>
        </div>
        <div class="modal-section-title"><i class="fa-solid fa-location-dot"></i> Address Details</div>
        <div class="detail-grid">
        ...
        <div class="docs-grid">
          <div class="doc-item">
            <img src="./images/{father_aadhar_image}" alt="Aadhar" onerror="this.src='images/noimage.png'">
            <p>Father Aadhar</p>
          </div>
          <div class="doc-item">
            <img src="./images/{parent_profile}" alt="Profile" onerror="this.src='images/noimage.png'">
            <p>Parent Profile</p>
          </div>
        </div>
        <div class="modal-section-title"><i class="fa-solid fa-key"></i> Login Credentials</div>
        <div class="cred-box">
          ...
          <div class="cred-note"><i class="fa-solid fa-circle-info"></i>&nbsp; These credentials were generated during registration and will be emailed on approval.</div>
        </div>
        <input type="hidden" name="cid" value="{parent_id}">
      </div>
      <div class="modal-footer">
        <button type="button" class="btn-cancel" onclick="closeModal('modal{parent_id}')"><i class="fa-solid fa-xmark"></i> Cancel</button>
        <button type="submit" name="reject" value="Reject" class="btn-reject"><i class="fa-solid fa-ban"></i> Reject</button>
        <button type="submit" name="approve" value="Approve" class="btn-approve"><i class="fa-solid fa-check"></i> Approve &amp; Send Email</button>
      </div>
    </form>
  </div>
</div>""")
...
function showSection(id) {
  ...
  const titles      = { 'sec-pending': 'Pending Parents', 'sec-children': 'Child Details' };
  const breadcrumbs = { 'sec-pending': 'Parents &rsaquo; Pending', 'sec-children': 'Children &rsaquo; Child Details' };
  document.getElementById('page-title').textContent = titles[id] || 'Pending Parents';
