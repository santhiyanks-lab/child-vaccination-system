...
cur.execute("SELECT * FROM parent WHERE status='Approved' ORDER BY parent_id DESC")
rows = cur.fetchall()

# Unique states
cur.execute("SELECT DISTINCT state FROM parent WHERE status='Approved' AND state IS NOT NULL ORDER BY state")
all_states = [r[0] for r in cur.fetchall()]

# State-district pairs for dynamic filtering
cur.execute("SELECT DISTINCT state, district FROM parent WHERE status='Approved' AND state IS NOT NULL AND district IS NOT NULL ORDER BY state, district")
...
.fsel{{padding:8px 12px;border:1.5px solid var(--bo);border-radius:8px;font-family:var(--fn);font-size:.86rem;background:var(--ca);color:var(--tx);cursor:pointer;min-width:130px}}
...
.pcard{{background:var(--ca);border-radius:var(--ra);box-shadow:var(--sh);border:1px solid var(--bo);margin-bottom:14px;overflow:hidden}}
.pcard-head{{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 18px;flex-wrap:wrap;transition:background var(--tr)}}
.pcard-head:hover{{background:#fafbff}}
.pcard-left{{display:flex;align-items:center;gap:14px}}
.av{{width:44px;height:44px;border-radius:50%;background:var(--pl);color:var(--p);display:flex;align-items:center;justify-content:center;font-size:1.1rem;font-weight:800;flex-shrink:0;border:2px solid var(--p)}}
.pcard-name{{font-size:.96rem;font-weight:700;color:var(--tx)}}
.pcard-sub{{font-size:.75rem;color:var(--mu);margin-top:3px}}
.pcard-right{{display:flex;gap:8px;align-items:center;flex-wrap:wrap}}
...
.btn-vm{{display:inline-flex;align-items:center;gap:6px;padding:7px 16px;background:var(--p);color:#fff;border:none;border-radius:7px;font-size:.82rem;font-weight:600;font-family:var(--fn);cursor:pointer;transition:background var(--tr)}}
.btn-vm:hover{{background:var(--pd)}}
.pcard-body{{display:none;border-top:1px solid var(--bo);background:#fafbff}}
.pcard-body.open{{display:grid;grid-template-columns:repeat(3,1fr)}}
.psec{{padding:16px 18px;border-right:1px solid var(--bo)}}
.psec:last-child{{border-right:none}}
.psec-title{{font-size:.71rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--p);margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid var(--pl);display:flex;align-items:center;gap:6px}}
...
@media(max-width:1024px){{
  .pcard-body.open{{grid-template-columns:1fr 1fr}}
...
@media(max-width:768px){{
  ...
  .pcard-body.open,.ccard-body.open{{grid-template-columns:1fr}}
  .psec,.csec{{border-right:none;border-bottom:1px solid var(--bo)}}
  .psec:last-child,.csec:last-child{{border-bottom:none}}
  ...
  <nav class="snav">
    <div class="ng open" id="g1">
      ...
      <div class="nsub">
        ...
        <a href="adminapprovedparent.py" class="active"><i class="fa-solid fa-check"></i> Approved {nb(approved_parents,"gr")}</a>
...
<div class="mwrap">
  <header class="topbar">
    <div class="tbl">
      ...
      <span class="ttitle" id="page-title">Approved Parents</span>
    </div>
    <div class="tbr">
      {tp}{th_alert}
...
<div class="section active" id="sec-parents">
  <div class="page-hdr">
    <div class="page-hdr-left">
      <div class="page-hdr-icon"><i class="fa-solid fa-user-check"></i></div>
      <div>
        <h2>Approved Parents</h2>
        <p>Filter by state and district, or search by name / email / mobile.</p>
      </div>
    </div>
    <span class="cnt-badge" id="pCntBadge"><i class="fa-solid fa-check-circle"></i> {len(rows)} Approved</span>
  </div>
  ...
  <div class="filter-bar">
    ...
    <div class="sbox">
      ...
      <input type="text" id="pSearch" placeholder="Search name, email, mobile..." oninput="filterParents()">
    </div>
    <select class="fsel" id="pStateFilter" onchange="updateDistricts('pStateFilter','pDistrictFilter'); filterParents()">
      {state_opts}
    </select>
    <select class="fsel" id="pDistrictFilter" onchange="filterParents()">
      {district_opts}
    </select>
    <button class="btn-clear" onclick="clearParentFilters()"><i class="fa-solid fa-xmark"></i> Clear</button>
    <span class="cnt-lbl" id="pCntLbl">{len(rows)} records</span>
  </div>
""")
...
if not rows:
    print('<div class="empty-state"><i class="fa-solid fa-users-slash"></i><p>No approved parents found.</p></div>')
else:
    for idx, i in enumerate(rows, 1):
        ...
        username    = i[15] or "—"
        password    = i[16] or "—"
        initial     = fname[0].upper() if fname != "—" else "P"

        aadhar_tag  = f'<img src="./images/{aadhar_img}" class="img-thumb" alt="Aadhar">'   if aadhar_img  else no_img
        profile_tag = f'<img src="./images/{profile_img}" class="img-thumb" alt="Profile">' if profile_img else no_img

        print(f"""
  <div class="pcard"
       data-search="{fname.lower()} {email.lower()} {mobile}"
    ...
    <div class="pcard-head">
      <div class="pcard-left">
        ...
        <div>
          <div class="pcard-name">{fname}</div>
          <div class="pcard-sub">
            <i class="fa-solid fa-envelope" style="font-size:.72rem"></i> {email} &nbsp;|&nbsp;
            <i class="fa-solid fa-phone" style="font-size:.72rem"></i> {mobile} &nbsp;|&nbsp;
      ...
      <div class="pcard-right">
        ...
        <button class="btn-vm" onclick="toggleDetail(this,'pd{pid}')">
          <i class="fa-solid fa-chevron-down"></i> View More
        </button>
      </div>
    </div>
    <div class="pcard-body" id="pd{pid}">
      <div class="psec">
        <div class="psec-title"><i class="fa-solid fa-user"></i> Personal Details</div>
        <div class="dg">
          <div class="di"><label>Father Name</label><span>{fname}</span></div>
          <div class="di"><label>Father DOB</label><span>{father_dob}</span></div>
          <div class="di"><label>Mother Name</label><span>{mname}</span></div>
          <div class="di"><label>Mother DOB</label><span>{mother_dob}</span></div>
          <div class="di"><label>Mother Weight</label><span>{mweight} kg</span></div>
          <div class="di"><label>Occupation</label><span>{occ}</span></div>
          <div class="di"><label>Email</label><span style="font-size:.8rem">{email}</span></div>
          <div class="di"><label>Mobile</label><span>{mobile}</span></div>
        </div>
      </div>
      <div class="psec">
        <div class="psec-title"><i class="fa-solid fa-location-dot"></i> Address &amp; Login</div>
        <div class="dg">
      ...
      <div class="psec">
        <div class="psec-title"><i class="fa-solid fa-file-image"></i> Documents &amp; Profile</div>
        <div class="dg">
          <div class="di"><label>Father Aadhar</label>{aadhar_tag}</div>
          <div class="di"><label>Profile Photo</label>{profile_tag}</div>
          <div class="di"><label>Parent ID</label><span>#{pid}</span></div>
...
function showSection(id){
  ...
  const titles={'sec-parents':'Approved Parents','sec-children':'Child Details'};
  document.getElementById('page-title').textContent=titles[id]||'Approved Parents';
...
/* ── Dynamic district filter based on selected state ── */
function updateDistricts(stateSelId, distSelId) {
  ...
  Array.from(distSel.options).forEach(opt => {
    if (!opt.value) return; // always keep "All Districts"
    opt.style.display = (!st || opt.dataset.state === st) ? '' : 'none';
  });
  // If current district selection no longer belongs to selected state, reset it
...
function filterParents(){
  ...
  const st  = document.getElementById('pStateFilter').value.toLowerCase();
  const di  = document.getElementById('pDistrictFilter').value.toLowerCase();
  let visible = 0;
  ...
  document.getElementById('pCntLbl').textContent  = visible + ' record' + (visible!==1?'s':'');
  document.getElementById('pCntBadge').innerHTML  = '<i class="fa-solid fa-check-circle"></i> ' + visible + ' Approved';
}
function clearParentFilters(){
  document.getElementById('pSearch').value = '';
  document.getElementById('pStateFilter').value = '';
  updateDistricts('pStateFilter','pDistrictFilter');
  document.getElementById('pDistrictFilter').value = '';
  filterParents();
}
